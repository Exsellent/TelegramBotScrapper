...
- **Режимы уведомлений**:
  ...
  - `digest`: Уведомления накапливаются в Redis и отправляются дайджестом (по умолчанию в 10:00, настраивается через
    app.batch.notification-cron`).

### Тегирование ссылок

- Пользователи добавляют теги к ссылкам (например, "Работа", "Хобби") через `/track` (опционально, можно пропустить
  с помощью "skip").
...
Тесты используют **Testcontainers** для PostgreSQL, Kafka и Redis, и **WireMock** для имитации внешних API
...
│       │       ├── dao/                # DAO-интерфейсы
│       │       ├── database/           # Базы данных и планировщик
│       │       ├── domain/             # JPA-сущности
│       │       ├── dto/                # DTO-объекты
│       │       ├── filter/             # Фильтры
